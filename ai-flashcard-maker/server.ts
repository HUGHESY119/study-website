...
async function generateContentWithRetry(ai: GoogleGenAI, options: any, maxRetries = 3, initialDelay = 1200): Promise<any> {
  const primaryModel = options.model || "gemini-3.5-flash";
  // Fallbacks: Try alternative lightweight/congest-free models if the primary model is overloaded
  const fallbackModels = ["gemini-2.5-flash", "gemini-1.5-flash", "gemini-3.1-flash-lite", "gemini-flash-latest"];
  ...
  for (const model of modelsToTry) {
    if (model !== primaryModel) {
      console.warn(`[Gemini API] Switching to fallback model: "${model}" due to overload of previous model.`);
    }

    let attempt = 0;
    while (attempt < maxRetries) {
      try {
        // Deep clone options to avoid SDK side-effects mutating parameters across attempts/models
...
async function extractTextFromFile(filePath: string, mimeType: string): Promise<string> {
  ...
  // Otherwise, use Gemini 3.5 Flash to extract the text contents (highly capable with PDFs and Images)
